...
class AstraProjectorSimple_cupy(LinearOperator):
    """ASTRA projector modified to use DataSet and geometry."""
    def __init__(self, geomv, geomp, device):
        super(AstraProjectorSimple_cupy, self).__init__()
...
if __name__  == '__main__':
    ...
    from ccpi.framework import AcquisitionGeometry_cupy as AcquisitionGeometry
    import numpy as np
    import cupy as cp
    N = 30
    angles = cp.linspace(0, np.pi, 180)
    ig = ImageGeometry(N, N)
    ag = AcquisitionGeometry('parallel','2D', angles, pixel_num_h = N)
    A = AstraProjectorSimple_cupy(ig, ag, 'gpu')
